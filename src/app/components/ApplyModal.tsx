...
type StepData = {
  ...
  propertyPhotoFiles?: File[];
  electricalPhotoFiles?: File[];
  projectPortfolioFiles?: File[];
    ...
    <span className="text-[10px] text-red-500 mt-1">
      {errors.chargersAvailable}
        ...
        <div className="relative flex flex-col">
          ...
          {isHoursDropdownOpen && (
            <div
              className="mt-3 border border-[#FFFFFF1A] rounded-xl bg-[#0000001A] backdropწ
...
function Step3Profile({
  ...
  const handleFile = (
    ...
    update({ [key]: [...((data[key] as File[]) || []), ...filesArray] });
  };
  ...
  const validateFields = () => {
    ...
    if (data.utilityBillUploaded === undefined)
      newErrors.utilityBillUploaded = t("apply_modal_required_field");
    if (
      data.utilityBillUploaded &&
      (!data.utilityBillFiles || data.utilityBillFiles.length === 0)
    )
  ...
  useEffect(() => {
    ...
    if (data.utilityBillUploaded !== undefined)
      delete newErrors.utilityBillUploaded;
    if (
      !data.utilityBillUploaded ||
      (data.utilityBillUploaded && data.utilityBillFiles?.length)
    )
      ...
      <div className="flex flex-col gap-y-[8px]">
        <div className="flex flex-col gap-y-[10px]">
          <p className="text-[12px] mb-2">
            ...
            <span className="text-[10px] italic ml-2">ход</span>
          </p>
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              type="radio"
          ...
          <div className="flex flex-col gap-3">
            ...
            {data.utilityBillFiles?.length ? (
              <div className="text-[12px] text-gray-300">
                {data.utilityBillFiles.map((file, index) => (
...
async function submitFinal(
  ...
  try {
    ...
    const uploadGroups = [
      { key: "utilityBillFiles" as const, files: data.utilityBillFiles || [] },
      {
    ...
    await Promise.all(
      uploadGroups.map(async ({ key, files }) => {
        ...
        if (!response.ok) throw new Error(`Upload failed for ${key}`);
    ...
    // Set done to true only on successful submission
    setDone(true);
  } catch (error) {
        ...
        {data.propertyPhotoConsent && (
          <div className="flex flex-col gap-3">
            ...
            {data.propertyPhotoFiles?.length ? (
              <div className="text-[12px] text-gray-300">
                {data.propertyPhotoFiles.map((file, index) => (
                  <div key={index}>{file.name}</div>
                ))}
              </div>
            ) : null}
        ...
        {data.electricalPhotoConsent && (
          <div className="flex flex-col gap-3">
            ...
            {data.electricalPhotoFiles?.length ? (
              <div className="text-[12px] text-gray-300">
                {data.electricalPhotoFiles.map((file, index) => (
                  <div key={index}>{file.name}</div>
                ))}
              </div>
            ) : null}
      ...
      <div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {data.projectPortfolioFiles?.length ? (
            <div className="text-[12px] text-gray-300">
              {data.projectPortfolioFiles.map((file, index) => (
                <div key={index}>{file.name}</div>
              ))}
            </div>
          ) : null}
